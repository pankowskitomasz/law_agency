...
class TTextList{
    ...
    private function descExists($descA){
        $sql = "select id from ".DB_PREFIX."cases where description=:pdesc";
        $stmt = $this->link->prepare($sql);
    ...
    private function titleExists($nameA){
        $sql = "select id from ".DB_PREFIX."cases where title=:ptitle";
        $stmt = $this->link->prepare($sql);
    ...
    public function getList(){
        $stmt = $this->link->prepare("select * from ".DB_PREFIX."cases");
        $stmt->execute();
        $res = $stmt->fetchAll();
        return $res;    
    }
    ...
    public function getListLength(){
        $stmt = $this->link->prepare("select count(*) from ".DB_PREFIX."cases");
        $stmt->execute();
        $res = $stmt->fetchAll();
        return $res[0][0];    
    }
    ...
    public function insert($titleA,$catA,$msgA){
        if(!$this->checkPresence($titleA,$msgA)){
            $sql = "insert into ";
            $sql .= DB_PREFIX;
            $sql .= "cases(title,category,description) ";
    ...
    public function save(){
        if($this->id){
            //update existing
            $sql = "update ".DB_PREFIX."cases set ";
            $sql .= "title = :ptitle,";
            $sql .= "category = :pcat,";
        ...
        && !$this->descExists($this->data['description'])){
            //create new
            $sql = "insert into ".DB_PREFIX."cases(title,category,description)";
